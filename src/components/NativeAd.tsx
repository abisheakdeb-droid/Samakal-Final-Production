...
export default function NativeAd({ title, description, image, url, sponsor }: NativeAdProps) {
  return (
    <a
      ...
      className="group block bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-sm overflow-hidden transition-all duration-300"
    >
...
export function NativeAdCompact({ title, image, url, sponsor }: Omit<NativeAdProps, 'description'>) {
  return (
    <a
      ...
      target="_blank"
      rel="noopener noreferrer sponsored"
      className="group block bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-sm overflow-hidden transition-all"
    >
      <div className="bg-amber-50 dark:bg-amber-900/10 px-3 py-1.5 border-b border-amber-100 dark:border-amber-900/30">
        <span className="text-xs font-semibold text-amber-700 dark:text-amber-500">প্রচারিত</span>
      </div>
      <div className="flex gap-3 p-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{sponsor}</p>
        </div>
      </div>
    </a>
  );
