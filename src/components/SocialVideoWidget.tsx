...
export default function SocialVideoWidget() {
  ...
  useEffect(() => {
    import('@/lib/actions-media').then(module => {
      module.fetchVideoArticles(4).then(videos => {
        setYtVideos(videos.map(v => ({
          id: v.relatedVideo?.id || v.id,
          title: v.title,
          thumb: v.relatedVideo?.thumbnail || v.image,
          duration: "০৫:০০" // Fallback duration
        })));
        setLoading(false);
      });
    });
